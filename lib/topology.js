...
Topology.prototype.purge = function (done) {
    ...
    let purgeKeys = this.opts.purgeKeys || [];
    purgeKeys = Array.isArray(purgeKeys) ? purgeKeys : [purgeKeys];
    ...
    async.each(
        ...
        function (err) {
            ...
            async.each(
                purgeKeys,
                function (purgeKey, next) {
                    const fullKey = `${prefix}${purgeKey}`;
                    self.opts.logger.log(
                        'info',
                        pad.timestamp(),
                        `Purging: ${fullKey}`
